package com.commencis.interview.mobile.locator;

import io.appium.java_client.AppiumBy;
import org.openqa.selenium.By;

/**
 * Views > Controls > 1. Light Theme ekranina ait locator'lar.
 *
 * <p>Bu ekrandaki her kontrolun kendi resource-id'si vardir, bu yuzden hepsinde id kullanilir.
 * Menu satirlarinda durum farklidir: orada tum satirlar android:id/text1 id'sini paylasir,
 * ayirt edici olan content-desc'tir (bkz. {@link ApiDemosLocators}).
 */
public final class ControlsLocators implements Locators {

    public static final String PAGE_NAME = "Views > Controls > 1. Light Theme";

    /** Uygulamanin id on eki; her locator'da tekrar yazilmasin diye ayrildi. */
    private static final String ID_PREFIX = "io.appium.android.apis:id/";

    public static final By TEXT_FIELD = AppiumBy.id(ID_PREFIX + "edit");

    /** Ekranda iki Save butonu var: bu etkin olan. */
    public static final By SAVE_BUTTON = AppiumBy.id(ID_PREFIX + "button");
    /** Ikinci Save butonu bilerek devre disi birakilmistir; negatif dogrulama icin kullanilir. */
    public static final By DISABLED_SAVE_BUTTON = AppiumBy.id(ID_PREFIX + "button_disabled");

    public static final By CHECKBOX_1 = AppiumBy.id(ID_PREFIX + "check1");
    public static final By CHECKBOX_2 = AppiumBy.id(ID_PREFIX + "check2");
    public static final By STAR_CHECKBOX = AppiumBy.id(ID_PREFIX + "star");

    public static final By RADIO_1 = AppiumBy.id(ID_PREFIX + "radio1");
    public static final By RADIO_2 = AppiumBy.id(ID_PREFIX + "radio2");

    public static final By TOGGLE_1 = AppiumBy.id(ID_PREFIX + "toggle1");
    public static final By TOGGLE_2 = AppiumBy.id(ID_PREFIX + "toggle2");

    public static final By PLANET_SPINNER = AppiumBy.id(ID_PREFIX + "spinner1");

    /**
     * Dropdown'da secili gorunen deger. Spinner container'inin kendi text'i bostur; secili
     * deger icindeki TextView'da durur. android:id/text1 ekranda baska yerlerde de gectigi
     * icin childSelector ile spinner1'e kapsanir.
     */
    public static final By PLANET_SPINNER_VALUE = AppiumBy.androidUIAutomator(
            "new UiSelector().resourceId(\"" + ID_PREFIX + "spinner1\")"
                    + ".childSelector(new UiSelector().resourceId(\"android:id/text1\"))");

    private ControlsLocators() {
    }

    /**
     * Acilan dropdown dialogundaki secenek.
     *
     * <p>Bu spinner secenekleri bir dialog listesinde (android:id/select_dialog_listview)
     * gosterir; secenekler CheckedTextView'dir ve yalnizca metinleriyle ayirt edilir.
     */
    public static By planetOption(String planet) {
        return AppiumBy.androidUIAutomator("new UiSelector().text(\"" + planet + "\")");
    }
}
